...
\begin{enumerate}[label=(\roman*)]
	\item Generate a Random Poisson variable $X$ and initialized to 0. Calculate the probability $pp$ for $X=0$ which is $e^{-\lambda}$. 
	\item Generate a random number u between 0 and 1 by dividing (double)rand() by RAND\_MAX.
	While $u$ is greater than $pp$, iterate and calculate $X$.Update $pp$ according to poisson distribution to calculate the cumulative distribution for increasing values of $X$.
	\item When $u$ does not meet the condition the value of Poisson random variable $X$ is found.
